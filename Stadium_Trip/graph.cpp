...
//                       matrix[originVertex][index] != 0 )
    ...
    index = 0;
    verticesListIndex = 0;
    while( index < vertexList.size() && verticesListIndex < verticesList.size())
    {
        if(verticesList[verticesListIndex] == index)
        {
            if(parents[index] != -1)
            {
                output.append("Cost to ");
        ...
        index++;
        qDebug() << "Value of verticesListIndex: " << verticesList[verticesListIndex];
        qDebug() << "ListIndex: "           << index;
